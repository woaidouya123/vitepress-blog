# [998.最大二叉树II](https://leetcode.cn/problems/maximum-binary-tree-ii)
<span class="diff diff-medium">中等</span>
2022-08-31 22:23:30 `JavaScript`
## 题目
...
由题意可知，节点插入的位置需要满足两个条件
1. 小于父节点的值，大于所有子节点的值
2. 在整颗树的最右边

故可能存在两种情况
1. 当值大于根节点的值时，该节点为新的根，且原有根节点为该节点左子节点
2. 当值小于根节点的值时，不断递归节点的右子节点：
    - 如果寻找到节点值小于val的节点，则将其替换为新节点，原节点为新节点的左子节点;
    - 如果没有找到小于val的节点，即递归到右子节点为空的节点，则将该节点的右指针指向新节点；
...
var insertIntoMaxTree = function(root, val) {
    let pre = null, node = root;
    while(node && node.val > val){
        pre = node;
        node = node.right;
    }
    let last = new TreeNode(val, node, null);
    pre && (pre.right = last);
    return pre ? root : last;
};
```
