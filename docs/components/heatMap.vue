<template>
  <div class="calander-container">
    ...
    <div
      :class="{ month: true, 'show-axis': props.showAxis }"
      v-for="mon in monthsMap.keys()"
      :style="{ width: Math.ceil((monthsMap.get(mon)?.length || 0) / 7) * 11 + 'px' }"
    >
...
const monthsMap: Map<string, Array<string>> = new Map()
const dataMap: object = props.data.reduce((pre, cur) => {
  pre[cur.date] = cur.count
  return pre
}, {})
while (startDate.valueOf() <= curDate.valueOf()) {
  const date = moment(startDate).format('YYYY-MM-DD')
  const mon = date.slice(0, 7)
  const arr = monthsMap.get(mon) || new Array(startDate.weekday()).fill('')
  arr.push(date)
...
const calcColor = (count) => {
  count = count || 0
  const index = props.level.findIndex((v) => count < v)
